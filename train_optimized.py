...
def parse_args():
    """解析命令行参数"""
    parser = argparse.ArgumentParser(description='DeepBindNet优化训练脚本')
    ...
    parser.add_argument('--huber_delta', type=float, default=1.0,
                        help='Huber损失的delta参数')
    parser.add_argument('--use_weighted_loss', action='store_true',
                        help='是否使用样本加权损失')
    parser.add_argument('--weight_type', type=str, default='abs_diff',
                        choices=['abs_diff', 'squared_diff', 'exp_diff'],
                        help='权重计算方式')
...
def get_loss_function(args):
    ...
    if args.loss_type == 'mse':
        ...
        base_criterion = nn.HuberLoss(delta=args.huber_delta)
    elif args.loss_type == 'log_cosh':
        base_criterion = LogCoshLoss()
    else:
    ...
    if args.use_weighted_loss:
        ...
        print(f"使用加权损失函数: {args.loss_type} + {args.weight_type} 加权 (alpha={args.weight_alpha})")
    else:
...
def train_epoch(model, train_loader, criterion, optimizer, device, epoch, writer, args, scaler=None):
    ...
    print(f'Epoch {epoch+1} - Train Loss: {epoch_loss:.4f}, RMSE: {rmse:.4f}, '
          f'R2: {r2:.4f}, MAE: {mae:.4f}, Time: {epoch_time:.2f}s')
...
def validate(model, val_loader, criterion, device, epoch, writer):
    ...
    # 打印验证摘要
    print(f'Validation - Loss: {val_loss:.4f}, RMSE: {rmse:.4f}, R2: {r2:.4f}, MAE: {mae:.4f}')
    
    return val_loss, rmse, r2, mae
...
def test(model, test_loader, criterion, device):
    ...
    # 打印测试摘要
    print(f'Test - Loss: {test_loss:.4f}, RMSE: {rmse:.4f}, R2: {r2:.4f}, MAE: {mae:.4f}')
...
def main():
    ...
    for epoch in range(args.num_epochs):
        # 训练一个轮次
        train_loss, train_rmse, train_r2, train_mae = train_epoch(
            model, train_loader, criterion, optimizer, device, epoch, writer, args, scaler
        )
        
        # 验证
        val_loss, val_rmse, val_r2, val_mae = validate(
    ...
    # 保存测试结果
    test_results['range_analysis'] = range_analysis
    with open(os.path.join(args.output_dir, 'test_results.pkl'), 'wb') as f:
        pickle.dump(test_results, f)
    
    # 打印最终结果
    print(f"\n训练完成! 最佳验证RMSE: {best_val_rmse:.4f}, 测试RMSE: {test_results['rmse']:.4f}")
